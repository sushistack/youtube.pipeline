...
**SCL-5** — Trigger: dogfood at `scp-049-run-4` (`stage=image`, 10 segments) showed an empty master pane despite populated segments — contradicting the §Approach line "master-detail when scenes exist" and the Direction B mockup, which place the scene list on every post-Phase-A surface. Amendment: §I/O Matrix is augmented with a new row "non-review stage WITH scenes" → master = scene list (read-only at non-HITL surfaces), detail = `DetailPanel` for the selected scene (already action-less by construction; approve/reject/regen affordances live in `BatchReview`'s sibling action bar). `SceneService.ListScenes` no longer gates on stage and runs unconditionally once the run row exists; `EditNarration` keeps its strict `scenario_review/waiting` gate so reads loosen but writes do not. The `/scenes` HTTP envelope is widened to the same shape as `/review-items` (rich critic_breakdown, shots, review_status) so non-batch surfaces render `SceneCard` and `DetailPanel` without a second fetch — `previous_version`, `prior_rejection`, and `high_leverage` stay zero-valued at non-batch_review stages because their assembly is batch_review-context-only. `service.parseCriticBreakdown` is exported as `service.ParseCriticBreakdown` (and a sibling `service.NormalizeOptionalScore`) so the new handler mapping reuses the same 0..1→0..100 normalization. Implemented under `fix/scenes-visible-everywhere` (spec: `_bmad-output/implementation-artifacts/spec-scenes-visible-everywhere.md`). KEEP: scenario_review/waiting and batch_review/waiting interactive surfaces unchanged; `BatchReview` continues to render full-bleed without the new master wrapper; the slim `narrationEditResponse` shape on POST `/scenes/{idx}/edit` (the inline editor's mutation cache update merges narration into the cached rich entry rather than replacing it). Deviation from the originating prompt: no `scenario_path → segments` backfill is added because none existed before this change — the prompt's "keep existing backfill" line described code that never shipped.
